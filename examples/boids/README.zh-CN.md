...
- **1 线程：** 5000/10000 只 boid 时，ekit 的测量结果快约 15-18%；稀疏集密集数组和直接查询分发可能是影响因素。
- **4 线程：** 5000/10000 只 boid 时，EnTT 的测量结果快约 30%。它按实体分块的 `parallel_for` 将实体分配给多个工作线程，而 ekit 按完整系统并行，并在每步执行线程池同步与依赖分析。
- 2-3 线程时两者基本相当。
